<x-app-layout>
    <div class="pt-28 mx-5">
         <div class="flex items-center mb-6 gap-x-1">
            ...
            <button id="kerjasama" type="button" class="btn btn-sm rounded-sm bg-green-500 text-green-900 hover:bg-green-600 hover:text-white"><i class="ri-add-fill text-lg"></i> KERJASAMA</button>
        </div>

        <form action="{{ route('kerjasama.store')}}" enctype="multipart/form-data" method="post" id="form" class="hidden items-center">
            @csrf
            @method('POST')
            <label class="form-control">
                <div class="label">
                        <span class="label-text required">Foto Kerjasama</span>
                </div>
                <div class="w-3/6 flex justify-between items-center mb-2 gap-x-2">
                    <div>
                        ...
                        <x-input-error :messages="$errors->get('img')" class="mt-2" />
                            <input type="text" name="name" id="name" placeholder="Nama Kerjasama.." class="input input-bordered input-sm w-full my-2">
